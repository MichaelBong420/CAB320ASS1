...
class Mine(search.Problem):
    ...
    def __init__(self, underground, dig_tolerance = 1):
        ...
        if underground.ndim == 2:
            self.len_z = underground.shape[1]
            self.initial = np.zeros(self.len_x)
        else:
            self.len_y = underground.shape[1]
            self.len_z = underground.shape[2]
    ...
    def actions(self, state):
       ...
       - surface_neighbours
        ...
        '''
        for loc in self.surface_neigbhours:
    ...
    def result(self, state, action):
        ...
        action in the given state. The action must be a valid action.
    ...
    def payoff(self, state):
        ...
        inbetween = state.self.underground

        payoff = np.cumsum(np.cumsum(inbetween, axis = 1), axis = 0)
    ...
    def is_dangerous(self, state):
        ...
        No loops needed in the implementation!
        '''
        # convert to np.array in order to use np operators
        state = np.array(state)
...
def search_dp_dig_plan(mine):
    ...
    '''
    best_payoff = mine.payoff(mine)
    best_action_list = []
    best_final_state = mine
    for action in mine.actions:
        child = mine.result(mine, action)
        child_payoff, child_action, child_final = search_dp_dig_plan(child)
        # this search_dp call won't work unless we yield actions 
        # (use a generator)
        if child_payoff > best_payoff:
            best_payoff = child_payoff
            best_action_list = child_action
            best_final_state = child_final
