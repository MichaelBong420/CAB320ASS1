...
class Mine(search.Problem):
    ...
    def __init__(self, underground, dig_tolerance = 1):
        ...
        if underground.ndim == 2:
            ...
            self.initial = numpy.zeros(self.len_x)
        else:
            self.len_y = underground.shape[1]
            self.len_z = underground.shape[2]
            self.initial = numpy.zeros([self.len_x, self.len_y])

        self.cumsum_mine = numpy.cumsum(underground, axis = 1)
    ...
    def payoff(self, state):
        ...
        payoff = numpy.cumsum(numpy.cumsum(inbetween, axis = 1), axis = 0)
